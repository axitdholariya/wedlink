...
export default function RoyalCourtyardTemplate() {
  ...
  // Default Wedding Data (Fallback agar localStorage na mile)
  ...
  // 1. AUTO-SYNC FROM BUILDER (Preview click hote hi yahan saari details update ho jayengi)
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('wedlink_official_builder') || localStorage.getItem('wedlink_shaadipath_builder');
      if (saved) {
        try {
          const parsed = JSON.parse(saved);
          setWeddingData(prev => ({ ...prev, ...parsed }));
        } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-[#FFF9F2] text-[#241C24] font-['Manrope',sans-serif] flex justify-center selection:bg-[#541D36] selection:text-[#FFF9F2]">
      ...
      <main className="w-full max-w-[480px] bg-[#FFF9F2] min-h-screen relative shadow-2xl overflow-x-hidden pb-12 border-x border-[#B68A50]/20">
        ...
        {/* ============================================================== */}
        {!isOpened && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 max-w-[480px] mx-auto transition-all duration-700">
            <div className="w-full h-full bg-[#541D36] flex flex-col items-center justify-center p-6 text-center text-[#FFF9F2] relative border-4 border-[#B68A50]/40">
              
              <div className="w-20 h-20 rounded-full bg-white/10 border-2 border-[#B68A50] flex items-center justify-center text-4xl mb-4 shadow-xl">
                🏰
              </div>
              ...
              <button
                ...
                className="mt-8 px-8 py-3.5 rounded-full bg-[#B68A50] hover:bg-[#c99a5e] text-[#541D36] font-bold text-xs uppercase tracking-[0.25em] shadow-xl transition-all flex items-center gap-2 cursor-pointer"
              >
        ...
        {/* 1. HERO SECTION                                                */}
        {/* ============================================================== */}
        <section className="pt-12 pb-8 px-6 text-center bg-gradient-to-b from-[#541D36] via-[#652342] to-[#FFF9F2] text-[#FFF9F2]">
          ...
          {weddingData.showCountdown && (
            <div className="mt-6 p-4 rounded-2xl bg-[#541D36]/90 border border-[#B68A50]/40 shadow-xl max-w-xs mx-auto">
        ...
        <section className="py-6 px-6">
          ...
          <div className="space-y-4">
            {weddingData.eventsList.filter(ev => weddingData.selectedEvents.includes(ev.id)).map((ev, i) => (
              <div key={ev.id} className="p-5 rounded-2xl bg-white border border-[#B68A50]/25 shadow-sm space-y-2">
        ...
        <section className="py-6 px-6 text-center">
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="w-full py-4 rounded-xl bg-[#541D36] hover:bg-[#682443] text-[#FFF9F2] font-bold text-xs uppercase tracking-[0.2em] shadow-xl flex items-center justify-center gap-2 transition-all cursor-pointer"
          >
